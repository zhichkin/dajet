...
> Изначально **DaJet Script** не проектировался для использования в подобных сценариях. Тем не менее возможность вызова скриптов по HTTP была заложена в него с целью развивать эту функциональность в будущем. В конечном итоге, в сочетании с командой ```REQUEST```, **DaJet Script web api** показал себя вполне достойно.
