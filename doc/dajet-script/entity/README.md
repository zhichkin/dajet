...
Для удобства работы с типом ```entity``` в DaJet Script реализован набор встроенных функций. Эти функции позволяют создавать новые ссылки из простых типов ```number``` и ```uuid```, получать части структуры ```entity``` и так далее. Некоторые функции имеют перегруженные версии, то есть отличаются набором параметров.
