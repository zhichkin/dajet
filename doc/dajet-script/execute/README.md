...
```SQL
EXECUTE 'file://<script>' [DEFAULT <default-script>] [WITH <parameters>] [INTO <variable>]
```
...
Опция **DEFAULT** используется только в том случае, когда для основного скрипта указан шаблон. Если по шаблону файл не будет найден, то выполняется попытка выполнить скрипт по умолчанию (default). Если опция **DEFAULT** не указана или альтернативный скрипт не будет найден, то возникает исключительная ситуация (ошибка).
