...
> **Важно!** Данная технология гарантирует последовательсность согласно очерёдности выполнения команд INSERT при записи в соответствующую таблицу СУБД без учёта последовательности завершения транзакций, в которых они участвуют. Таким образом последовательность гарантируется в разрезе уникальных ключей таблицы СУБД, например, кластерного индекса. Если требуется последовательность именно **завершённых транзакций**, то следует использовать технологию CDC (change data capture), которая имеет доступ к LSN (log sequence number) журнала транзакций соответствующей СУБД. Данная технология хорошо подходит для объектного обмена данными или их изменениями, характерного, например, для платформы 1С:Предприятие 8.
