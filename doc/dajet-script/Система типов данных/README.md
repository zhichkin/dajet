...
> Ссылка, то есть значение типа ```entity```, всегда строго типизированна, то есть даже "пустая ссылка" указывает на отсутствие объекта какого-то определённого типа, например, "Справочник.Номенклатура".
