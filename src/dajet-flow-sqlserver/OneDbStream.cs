...
namespace DaJet.Flow.SqlServer
{
    [PipelineBlock] public sealed class OneDbStream : SourceBlock<IDataRecord>
    {
        ...
        {
            command.CommandText = ScriptCommands[0].Script;
            EntityMap mapper = ScriptCommands[0].Mapper;
        ...
        {
            int next = 0;
            command.CommandText = ScriptCommands[next].Script;
            EntityMap mapper = ScriptCommands[next++].Mapper;
            ...
            for (int i = 1; i < ScriptCommands.Count; i++)
            {
                script.AppendLine(ScriptCommands[i].Script);
            }
            ...
            foreach (DataRecord record in table)
            {
                ...
                using (IDataReader reader = command.ExecuteReader())
                {
                    do
                    {
                        ...
                        ScriptCommand sc = ScriptCommands[next++];
