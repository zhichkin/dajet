...
{
    public sealed class MsRequestProcessor : IProcessor
    {
        ...
        public MsRequestProcessor(in ScriptScope scope)
        {
        ...
        private void ProcessRecord(in SqlDataReader reader)
        {
        ...
        private void ConfigureQueryParameters(in SqlCommand command)
        {
        ...
        {
            using (SqlConnection connection = _factory.Create(in _uri) as SqlConnection)
            {
                connection.Open();
